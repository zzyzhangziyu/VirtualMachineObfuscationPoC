...
#### Registers
A register is a slot for storing value on the CPU. The VM has 10 total registers, each of which is 4 VBYTEs (32 bits). The six of them are general purpose, one has designated role as program counter and another has role as stack pointer. The VM has also two regisers ZF (Zero Flag) and CF (Carry Flag). These two provide information about the most recently executed calculation (allows to check logical conditions such as *AND*).
...
02  |  MOVMB r<sub>dst</sub>, addr<sub>src</sub> | Move and extend VBYTE from memory to a register|
03  |  MOVMW r<sub>dst</sub>, addr<sub>src</sub> | Move VWORD from memory to a register |
04  |  MOVB r<sub>dst</sub>, VBYTE | Move and extend VBYTE to a register  |
05  |  MOVW r<sub>dst</sub>, VWORD | Move VWORD to a register |
06  |  MOVBM addr<sub>dst</sub>, r<sub>src</sub> | Move VBYTE from a register to memory location |
07  |  MOVWM addr<sub>dst</sub>, r<sub>src</sub> | Move VWORD from a register to memory location |
08  |  MOVMRB r<sub>dst</sub>, r<sub>src</sub> | Move and extend VBYTE from memory to a register; get an address from a register |
09  |  MOVMRW r<sub>dst</sub>, r<sub>src</sub> | Move VWORD from memory to a register; get an address from a register |
0A  |  MOVMD r<sub>dst</sub>, addr<sub>src</sub> | Move double VWORD from memory to a register |
0B  |  MOVD r<sub>dst</sub>, VDWORD | Move double VWORD to a register |
0C  |  MOVDM addr<sub>dst</sub>, r<sub>src</sub> | Move double VWORD from a register to memory location |
...
26  |  JA addr | Jump if above; it set up PC to the specified location if the ZF is not set (0) and the CF is not set (0) |
  | | |
30  |  ADVR r<sub>dst</sub>, VWORD | Add VWORD value to a register |
31  |  ADRR r<sub>dst</sub>, r<sub>src</sub> | Add two registers |
32  |  ADRRL r<sub>dst</sub>, r<sub>src</sub> | Add two registers (the low VBYTE) |
33  |  SUBVR r<sub>dst</sub>, VWORD | Substract VWORD value from a register |
34  |  SUBRR r<sub>dst</sub>, r<sub>src</sub> | Substract two registers |
35  |  SUBRRL r<sub>dst</sub>, r<sub>src</sub> | Substract two registers (the low VBYTE) |
36  |  XOR r<sub>dst</sub>, r<sub>src</sub> | Xor two registers |
37  |  XOR r<sub>dst</sub>, r<sub>src</sub> | Xor two registers (the low VBYTE) |
38  |  NOT r<sub>dst</sub>| Bitwise NOT on value in a register |
39  |  NOT r<sub>dst</sub> | Bitwise NOT on value in a register (the low VBYTE) |
3A  |  ADVRD r<sub>dst</sub>, VDWORD | Add double VWORD value to a register |
3B  |  SUBVR r<sub>dst</sub>, VDWORD | Substract double VWORD value from a register |
3C  |  SHR r<sub>dst</sub>, count<sub>VBYTE</sub> | Shift the bits of the operand destination to the right, by the number of bits specified in the count operand |
3D  |  SHL r<sub>dst</sub>, count<sub>VBYTE</sub> | Shift the bits of the operand destination to the left, by the number of bits specified in the count operand |
  | | |
50  |  CMP r<sub>dst</sub>, r<sub>src</sub> | Compare two registers |
51  |  CMPL r<sub>dst</sub>, r<sub>src</sub> | Compare two registers (the low VBYTE) |
  | | |
60  |  SYSBUS id<sub>VBYTE</sub> | todo |
61  |  NETBUS id<sub>VBYTE</sub> | todo |
  | | |
...
93  |  SETSP VDWORD| Set the stack pointer to the double VWORD value |
  | | |
